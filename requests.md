...
The `request` object is bound into the IOC container on every request. This takes the WSGI environment variables generated by your WSGI server as a parameter. This is done by Masonite. This `Request` class is initialized inside the `bootstrap/start.py` file. We grab this request object by simply passing in `Request` into our controller method.
